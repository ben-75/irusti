...
pub fn char_to_trits(tryte: char) -> &'static [i8; TRITS_PER_TRYTE] {
    for (i, mapping) in TRYTE_TO_TRITS_MAPPINGS
        .iter()
        .enumerate()
        .take(TRYTE_ALPHABET.len())
        {
...
pub fn trytes(trits: &[i8]) -> String {
    ...
    for i in 0..(trits.len() + NUMBER_OF_TRITS_IN_A_TRYTE - 1) / NUMBER_OF_TRITS_IN_A_TRYTE {
        ...
        if j < 0 {
            j += TRYTE_ALPHABET.len() as i8;
        }
        trytes.push(TRYTE_ALPHABET[j as usize]);
    }
    trytes
}
