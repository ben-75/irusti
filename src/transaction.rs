...
//        if signature_message_fragment.len()>2187 {return Err("Message/Signature fragment is too long".to_string())};
//        if address.len()>81 {return Err("Address is too long".to_string())};
//
//        arr = register_trytes(signature_message_fragment,0,0,&mut arr).unwrap();
//        arr = register_trytes(address,SIG_MSG_BYTE_SIZE,SIG_MSG_BYTE_OVERFLOW,&mut arr).unwrap();
//        arr = register_i64(value,VALUE_BYTE_OFFSET,VALUE_BIT_OFFSET, & mut arr).unwrap();
...
//        }
//
//        let adr = to_bytes(signature_message_fragment);
//        match adr {
//            Err(x) => return Err(x.to_string()),
//            Ok(x) => {
//                arr[ADDRESS_TRINARY_OFFSET..x.len()].copy_from_slice(x.as_ref());
//            }
//        }
