...
pub fn i64_to_trits(value :i64) -> [i8;40]{
    ...
    if v > 2026277576509488133 { r[39] = 1; v += -4052555153018976267; }
    if v < -2026277576509488133 { r[39] = -1; v += 4052555153018976267; }
    if v > 675425858836496044 { r[38] = 1; v += -1350851717672992089; }
    if v < -675425858836496044 { r[38] = -1; v += 1350851717672992089; }
    if v > 225141952945498681 { r[37] = 1; v += -450283905890997363; }
    if v < -225141952945498681 { r[37] = -1; v += 450283905890997363; }
    if v > 75047317648499560 { r[36] = 1; v += -150094635296999121; }
    if v < -75047317648499560 { r[36] = -1; v += 150094635296999121; }
    if v > 25015772549499853 { r[35] = 1; v += -50031545098999707; }
    if v < -25015772549499853 { r[35] = -1; v += 50031545098999707; }
    if v > 8338590849833284 { r[34] = 1; v += -16677181699666569; }
    if v < -8338590849833284 { r[34] = -1; v += 16677181699666569; }
    if v > 2779530283277761 { r[33] = 1; v += -5559060566555523; }
    if v < -2779530283277761 { r[33] = -1; v += 5559060566555523; }
    if v > 926510094425920 { r[32] = 1; v += -1853020188851841; }
    if v < -926510094425920 { r[32] = -1; v += 1853020188851841; }
    if v > 308836698141973 { r[31] = 1; v += -617673396283947; }
    if v < -308836698141973 { r[31] = -1; v += 617673396283947; }
    if v > 102945566047324 { r[30] = 1; v += -205891132094649; }
    if v < -102945566047324 { r[30] = -1; v += 205891132094649; }
    if v > 34315188682441 { r[29] = 1; v += -68630377364883; }
    if v < -34315188682441 { r[29] = -1; v += 68630377364883; }
    if v > 11438396227480 { r[28] = 1; v += -22876792454961; }
    if v < -11438396227480 { r[28] = -1; v += 22876792454961; }
    if v > 3812798742493 { r[27] = 1; v += -7625597484987; }
    if v < -3812798742493 { r[27] = -1; v += 7625597484987; }
    if v > 1270932914164 { r[26] = 1; v += -2541865828329; }
    if v < -1270932914164 { r[26] = -1; v += 2541865828329; }
    if v > 423644304721 { r[25] = 1; v += -847288609443; }
    if v < -423644304721 { r[25] = -1; v += 847288609443; }
    if v > 141214768240 { r[24] = 1; v += -282429536481; }
    if v < -141214768240 { r[24] = -1; v += 282429536481; }
    if v > 47071589413 { r[23] = 1; v += -94143178827; }
    if v < -47071589413 { r[23] = -1; v += 94143178827; }
    if v > 15690529804 { r[22] = 1; v += -31381059609; }
    if v < -15690529804 { r[22] = -1; v += 31381059609; }
    if v > 5230176601 { r[21] = 1; v += -10460353203; }
    if v < -5230176601 { r[21] = -1; v += 10460353203; }
    if v > 1743392200 { r[20] = 1; v += -3486784401; }
    if v < -1743392200 { r[20] = -1; v += 3486784401; }
    if v > 581130733 { r[19] = 1; v += -1162261467; }
    if v < -581130733 { r[19] = -1; v += 1162261467; }
    if v > 193710244 { r[18] = 1; v += -387420489; }
    if v < -193710244 { r[18] = -1; v += 387420489; }
    if v > 64570081 { r[17] = 1; v += -129140163; }
    if v < -64570081 { r[17] = -1; v += 129140163; }
    if v > 21523360 { r[16] = 1; v += -43046721; }
    if v < -21523360 { r[16] = -1; v += 43046721; }
    if v > 7174453 { r[15] = 1; v += -14348907; }
    if v < -7174453 { r[15] = -1; v += 14348907; }
    if v > 2391484 { r[14] = 1; v += -4782969; }
    if v < -2391484 { r[14] = -1; v += 4782969; }
    if v > 797161 { r[13] = 1; v += -1594323; }
    if v < -797161 { r[13] = -1; v += 1594323; }
    if v > 265720 { r[12] = 1; v += -531441; }
    if v < -265720 { r[12] = -1; v += 531441; }
    if v > 88573 { r[11] = 1; v += -177147; }
    if v < -88573 { r[11] = -1; v += 177147; }
    if v > 29524 { r[10] = 1; v += -59049; }
    if v < -29524 { r[10] = -1; v += 59049; }
    if v > 9841 { r[9] = 1; v += -19683; }
    if v < -9841 { r[9] = -1; v += 19683; }
    if v > 3280 { r[8] = 1; v += -6561; }
    if v < -3280 { r[8] = -1; v += 6561; }
    if v > 1093 { r[7] = 1; v += -2187; }
    if v < -1093 { r[7] = -1; v += 2187; }
